...
export default {
    data() {
        return {
            ...
            checkYichehUrl: 'http://mapi.yiche.com/app_third/api/v1/enquiry/requirement_audit'   //审核之后易车惠接口
        }
    },
    created(){
        this.formatProjectStatus();
        this.getDetailData();    //获取项目详情数据
    },
    methods: {
        //返回上一级
        backPageFun() {
            this.$router.push({
                path:'/clueManage/clueList',
                query: {
                    status: this.status,
        ...
        getAppThirdData(num){
            this.api({
                ...
                this.$router.push({path:'/clueManage/clueList'})
            })
        }
    }
}
